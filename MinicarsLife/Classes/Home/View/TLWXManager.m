...
- (void)registerApp {
    ...
    [WXApi registerApp:@"wx14ce9b413e063005" universalLink:@"https://share.ios.fhcdzx.com/"];
}
...
+ (void)wxShareWebPageWithScene:(int)scene title:(NSString *)title desc:(NSString *)desc url:(NSString *)url {
    ...
    NSString *shareTitle = PASS_NULL_TO_NIL(title).length > 0 ? title : @"会玩车";
    NSString *shareDesc = PASS_NULL_TO_NIL(desc).length > 0 ? desc : @"欢迎使用会玩车";
    // -- //
    ...
    [message setThumbImage:kImage(@"512的副本 3")];
    //
