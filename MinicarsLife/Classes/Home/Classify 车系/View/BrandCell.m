...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.logo = [[UIImageView alloc]initWithFrame:CGRectMake((self.contentView.width - 40) / 2,(self.contentView.height) / 2 - 30, 40, 40)];
//        self.logo.image = kImage(@"1");
        self.logo.contentMode = UIViewContentModeScaleAspectFill;
        [self addSubview:self.logo];
        
        self.brandname = [UILabel labelWithFrame:CGRectMake(0, self.logo.yy + 5, self.width, 15) textAligment:(NSTextAlignmentCenter) backgroundColor:kClearColor font:Font(14) textColor:kBlackColor];
